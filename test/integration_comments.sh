...
# This check drives the theme's comment rendering through the template's demo
# blog posts. This site keeps no blog posts, so there is nothing to assert
# against; skip rather than fail on missing fixtures.
if [[ ! -f "${giscus_page}" || ! -f "${disqus_page}" ]]; then
  echo "comments integration checks skipped: no blog posts with comments in this site"
  exit 0
fi
